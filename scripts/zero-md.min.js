class t extends HTMLElement {
	...
	onload(t) {
		return new Promise((e, s) => {
			(t.onload = e),
				(t.onerror = (t) =>
					s(t.path ? t.path[0] : t.composedPath()[0]));
		});
	}
	loadScript(t) {
		return Promise.all(
	...
	highlight(t) {
		return new Promise((e) => {
			t.querySelectorAll('pre>code:not([class*="language-"])').forEach(
				(t) => {
	...
	async buildMd(t = {}) {
		return (
			...
			(() => {
				...
				if (!e) return "";
				const s = e.hasAttribute("data-dedent")
					? this.dedent(e.text)
					: e.text;
	...
	async stampBody(t, e) {
		...
		if (i.getAttribute("data-hash") !== r) {
			...
			return (
				(i.innerHTML = ""), i.append(...e), await this.highlight(i), !0
			);
		}
	}
	async render(t = {}) {
